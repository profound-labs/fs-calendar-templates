...
function monthMarksDayTextAsCornerMarks(monthName, filterPred, eventsCsv)
  ...
  local spacing = {
    full   = "-14.85pt and 11.00pt",
    waning = "-14.70pt and 11.15pt",
    new    = "-14.70pt and 11.15pt",
    waxing = "-14.70pt and 11.15pt"
  }
    ...
    if ok(event.phase) then
      ...
      if do_test_alignment then
        for k,v in pairs(phases) do
          tsp(string.format(" \\draw node [above left=%s of cal%s-%s.north east, opacity=%s] {%s}; ",
                            spacing[v],
                            d:fmt("%m"),
                            event.date,
                            opacity[v],
        ...
        tsp(string.format(" \\draw node [above left=%s of cal%s-%s.north east] {%s}; ",
                          spacing[event.phase],
                          d:fmt("%m"),
                          event.date,
                          event.day_text))
